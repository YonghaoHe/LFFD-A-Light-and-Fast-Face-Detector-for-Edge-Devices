...
def get_net_symbol(deploy_flag=False):
    ...
    conv20 = mxnet.symbol.Convolution(data=relu19,
                                      ...
                                      name='conv20')
    # Residual
    conv20 = conv18 + conv20
    relu20 = mxnet.symbol.Activation(data=conv20, act_type='relu', name='relu_conv20')

    # loss 5 RF:655 ----------------------------------------------------------------------------------------------------
    # for scale [160,320]----------------------------------------------------------------------------------------
    if deploy_flag:
        predict_score_5, predict_bbox_5 = loss_branch(relu20, 'conv20', deploy_flag=deploy_flag)
    else:
        loss_score_5, loss_bbox_5 = loss_branch(relu20, 'conv20', mask=mask_5, label=label_5)
