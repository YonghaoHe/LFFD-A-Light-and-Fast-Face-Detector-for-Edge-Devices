...
"""
import os
from config_farm import configuration_64_512_16L_3scales_v1_small as cfg
...
def get_net_symbol(deploy_flag=False):
    ...
    # for scale [64,128]----------------------------------------------------------------------------------------
    if deploy_flag:
        predict_score_1, predict_bbox_1 = loss_branch(relu10, 'conv10', deploy_flag=deploy_flag)
    else:
        loss_score_1, loss_bbox_1 = loss_branch(relu10, 'conv10', mask=mask_1, label=label_1)
    ...
    # for scale [128,256]----------------------------------------------------------------------------------------
    if deploy_flag:
        predict_score_2, predict_bbox_2 = loss_branch(relu13, 'conv13', deploy_flag=deploy_flag)
    else:
        loss_score_2, loss_bbox_2 = loss_branch(relu13, 'conv13', mask=mask_2, label=label_2)
    ...
    conv16 = mxnet.symbol.Convolution(data=relu15,
                                      ...
                                      num_filter=num_filters_list[2],
                                      name='conv16')
    conv16 = conv14 + conv16
    relu16 = mxnet.symbol.Activation(data=conv16, act_type='relu', name='relu_conv16')

    # loss 3 RF:703 ----------------------------------------------------------------------------------------------------
    # for scale [256,512]----------------------------------------------------------------------------------------
    if deploy_flag:
        predict_score_3, predict_bbox_3 = loss_branch(relu16, 'conv16', deploy_flag=deploy_flag)
    else:
        loss_score_3, loss_bbox_3 = loss_branch(relu16, 'conv16', mask=mask_3, label=label_3)
